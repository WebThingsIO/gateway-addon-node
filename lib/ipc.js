...
class IpcSocket {
  ...
  constructor(name, socketType, protocol, baseAddr, onMsg, appInstance) {
    ...
    const base_dir = path.resolve(path.join(__dirname, '..', 'schema'));
    const schemas = [];

    // top-level schema
    schemas.push(
      JSON.parse(fs.readFileSync(path.join(base_dir, 'schema.json')))
    );

    // individual message schemas
    for (const fname of fs.readdirSync(path.join(base_dir, 'messages'))) {
      schemas.push(
        JSON.parse(fs.readFileSync(path.join(base_dir, 'messages', fname)))
      );
    }
